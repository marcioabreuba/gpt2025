...
async function fetchAllShopifyOrders(instanceId = null) {
  try {
    // Obtém a configuração da loja com base no instanceId
    const shopConfig = await getShopifyConfigByInstanceId(instanceId);
    
    let allOrders = [];
    let nextPageUrl = `https://${shopConfig.shopDomain}/admin/api/2024-10/orders.json?status=any&limit=250`;
    let attempt = 0;
    const maxAttempts = 3;
...
router.post('/pedidos', async (req, res) => {
  try {
    ...
    const ordersData = await fetchAllShopifyOrders(instanceId);
    console.log(`🔄 Total de pedidos encontrados na loja ${ordersData.shopName}: ${ordersData.count}`);
